...
castle search "database decision" --wing myapp

# Filter by room (topic)
castle search "auth decisions" --room auth-migration

# Filter by both
castle search "pricing" --wing driftwood --room costs

# More results
castle search "deploy process" --results 10
```

## How Search Works

1. Your query is embedded using the vector store's default model (`BAAI/bge-m3` with the default LanceDB backend).
...
The AI calls `castle_search` behind the scenes. You never type a search command.

...
The Python package that powers Cognitive Castle. All modules, all logic.
...
| `config.py` | Configuration loading — `~/.castle/config.json`, env vars, defaults |
...
| `miner.py` | Project file ingest — scans directories, chunks by paragraph, stores to LanceDB |
...
The palace (LanceDB) stores verbatim content with dense and full-text indexes. The knowledge graph (SQLite) stores structured relationships. The MCP server exposes both to any AI tool.

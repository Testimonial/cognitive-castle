#!/usr/bin/env python3
"""
onboarding.py — Cognitive Castle first-run setup.

Asks the user:
  1. How they're using Cognitive Castle (work / personal / combo)
...
Seeds the entity_registry with confirmed data so Cognitive Castle knows your world
from minute one — before a single session is indexed.

Usage:
    python3 -m cognitive_castle.onboarding
    or: castle init
"""
...
def _ask_mode() -> str:
    _header("Welcome to Cognitive Castle")
    print("""
  Cognitive Castle is a personal memory system. To work well, it needs to know
...
""")
    print("  How are you using Cognitive Castle?")
    print()
...
def _ask_projects(mode: str) -> list:
    ...
    _hr()
    print("""
  What are your main projects? (These help Cognitive Castle distinguish project
...
def _warn_ambiguous(people: list) -> list:
    ...
    """
    from cognitive_castle.entity_registry import COMMON_ENGLISH_WORDS
...
def _generate_aaak_bootstrap(
    ...
    """
    castle_dir = Path(config_dir) if config_dir else Path.home() / ".castle"
    ...
    registry_lines = [
        "# AAAK Entity Registry",
        "# Auto-generated by castle init. Update as needed.",
        "",
        "## People",
    ]
    for p in people:
    ...
    (castle_dir / "aaak_entities.md").write_text("\n".join(registry_lines), encoding="utf-8")
    ...
    (castle_dir / "critical_facts.md").write_text("\n".join(facts_lines), encoding="utf-8")
...
def run_onboarding(
  ...
  Cognitive Castle will check the context before treating them as person names.
...
""")
    ...
    print("\n  AAAK entity registry: ~/.castle/aaak_entities.md")
    print("  Critical facts bootstrap: ~/.castle/critical_facts.md")

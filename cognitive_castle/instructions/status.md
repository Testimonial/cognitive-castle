# Cognitive Castle Status

Display the current state of the user's memory palace.

## Step 1: Gather Palace Status

Check if MCP tools are available (look for castle_status in available tools).

- If MCP is available: Call the castle_status tool to retrieve palace state.
- If MCP is not available: Run the CLI command: castle status
...
- castle_kg_stats -- for a knowledge graph overview (triple count, entity
  count, relationship types)
- castle_graph_stats -- for connectivity information (connected components,
...
- Empty palace (zero memories): Suggest "Try /castle:mine to add data from
  files, URLs, or text."
- Has data but no knowledge graph (memories exist but KG stats show zero
  ...
  queries."
- Healthy palace (has memories and KG data): Suggest "Use /castle:search to
  query your memories."

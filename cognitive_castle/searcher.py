...
def _hybrid_rank(
    ...
    * BM25 is real Okapi-BM25 with corpus-relative IDF over the candidates
    ...
    for r, raw, norm in zip(results, bm25_raw, bm25_norm):
        ...
        # Multiply by decay_score so stale drawers rank lower.
        # decay_score defaults to 1.0 (fresh); procedural/user memories never decay.
        decay = float((r.get("metadata") or {}).get("decay_score", 1.0))
        r["bm25_score"] = round(raw, 3)
        scored.append(((vector_weight * vec_sim + bm25_weight * norm) * decay, r))
...
def search_memories(
    ...
    for doc, meta, dist in zip(
        ...
        entry = {
            ...
            # Full metadata kept internally — used by _hybrid_rank (decay_score)
            # and the SOAR bridge; stripped before returning to callers.
            "metadata": meta,
    ...
    for h in hits:
        ...
        if len(expanded) > MAX_HYDRATION_CHARS:
            expanded = (
                ...
                "Use castle_get_drawer for full content.]"
            )
        h["text"] = expanded

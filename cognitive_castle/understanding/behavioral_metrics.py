...
class BehavioralAnalyzer:
    ...
    def extract_all_transitions(self, requirements: List[str]) -> List[Dict]:
        """
        Extract all behavioral transitions across multiple requirements.

        Calls extract_transitions() for each requirement and returns a flat
        list of transition dicts with requirement index.

        Args:
            requirements: List of requirement strings

        Returns:
            List of dicts with keys: guard, action, outcome, is_complete, requirement_index
        """
        result = []
    ...
    def analyze_requirements(self, requirements: List[str]) -> BehavioralMetrics:
        ...
        # Steepness k=3.0: 0.2 conditionals/req -> 0.45, 0.3 -> 0.59, 0.5 -> 0.78
        conditional_ratio = conditional_count / total_reqs
        scenario_decomposition_score = 1.0 - math.exp(-3.0 * conditional_ratio)

...
_SUBCOMMANDS = {"version", "scan", "--help", "-h", "--version"}

app = typer.Typer(
    name="understanding",
    help="Requirements quality metrics — 34 measures based on readability formulas, IEEE/ISO standards, and RE research",
...
def _version_callback(value: bool) -> None:
    if value:
        ...
        console.print("34 requirements quality metrics based on readability formulas, IEEE/ISO standards, and RE research")
        raise typer.Exit()
...
def scan(
    ...
    enhanced: bool = typer.Option(
        True,
        "--enhanced/--basic",
        help="Use all 34 metrics (enhanced) or base 18 metrics (basic)",
    ),
    ...
    Analyzes specifications using 34 metrics with NLP
        ...
        for spec_file in spec_files:
            if per_requirement:
                ...
                if parsed["count"] == 0:
                    ...
                    req_results = []
                    _sem_analyzer = SemanticAnalyzer(use_spacy=use_nlp)
                    _constraint_analyzer = ConstraintAnalyzer()
                    for req in parsed["requirements"]:
                        ...
                        req_result["ears_pattern"] = classify_ears_pattern(req["text"])
                        req_result["semantic_roles"] = _sem_analyzer.extract_roles_as_dict(req["text"])
                        req_result["constraint_diagnostics"] = _constraint_analyzer.diagnose_requirement(req["text"])
...
def _print_test_result(result: dict, per_requirement: bool = False):
    ...
    if per_requirement:
        ...
        if parsed["count"] == 0:
            ...
            # Analyze each requirement
            req_results = []
            _sem_analyzer = SemanticAnalyzer(use_spacy=True)
            _constraint_analyzer = ConstraintAnalyzer()
            for req in parsed["requirements"]:
...
def _print_result(result: dict):
    ...
    total = metric_count.get('total', 34 if enhanced else 18)
    if not enhanced:
...
def _print_entity_analysis(result: dict, show_diagram: bool = False, png_path: Optional[Path] = None, png_format: str = "png"):
    ...
    if png_path and relationships:
        ...
        if not GRAPHVIZ_AVAILABLE:
            ...
            success = extractor.generate_graphviz_diagram(
                entity_objs,
                relationship_objs,
                title=title,

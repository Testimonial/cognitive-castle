...
_nlp = None
_nlp_loaded = False
SPACY_AVAILABLE = None  # None = unknown, True/False set on first load attempt


def _get_nlp():
    """Lazy-load spaCy model on first use."""
    global _nlp, _nlp_loaded, SPACY_AVAILABLE
    if not _nlp_loaded:
        _nlp_loaded = True
        try:
            import spacy
...
class SemanticAnalyzer:
    ...
    def extract_semantic_roles_spacy(self, text: str) -> SemanticRoles:
        ...
        # If no triggers found via dependency, use regex fallback
        # spaCy dep_=="mark" misses when/if/unless tagged as advmod/advcl
        if not triggers:
            for pattern in self.TRIGGER_PATTERNS:
    ...
    def extract_roles_as_dict(self, text: str) -> dict:
        """Extract semantic roles and return as a plain dict.
...
def classify_ears_pattern(requirement_text: str) -> str:
    """Classify a requirement into an EARS category.

    EARS = Easy Approach to Requirements Syntax (Mavin et al., 2009)
    Categories: ubiquitous, event_driven, state_driven, optional, unwanted, unclassified
    """
    text_lower = requirement_text.lower()

    # Check for unwanted first (SHALL NOT / MUST NOT)
    if any(neg in text_lower for neg in ['shall not', 'must not', 'will not', 'should not']):
        return 'unwanted'

    # Check for event-driven (WHEN / IF triggers)
    if any(trigger in text_lower for trigger in ['when ', 'if ', 'upon ', 'after ', 'before ']):
        return 'event_driven'

    # Check for state-driven (WHILE)
    ...
    # Check for ubiquitous (positive SHALL/MUST with no trigger)
    if any(modal in text_lower for modal in ['shall ', 'must ', 'will ']):
        return 'ubiquitous'

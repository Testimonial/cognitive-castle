...
class ConstraintAnalyzer:
    ...
    def diagnose_requirement(self, text: str) -> Dict:
        """
        Produce per-requirement constraint diagnostics.

        Args:
            text: Single requirement text

        Returns:
            Dict with hard_constraint_count, soft_constraint_count,
            soft_words (matched keywords), and diagnosis string.
        """
        hard = self.count_hard_constraints(text)
        soft = self.count_soft_constraints(text)

        text_lower = text.lower()
        soft_words = [kw for kw in self.SOFT_CONSTRAINT_KEYWORDS if kw in text_lower]
    ...
    def analyze_requirements(self, requirements: List[str]) -> ConstraintMetrics:
        ...
        # Steepness k=2.0: 0.5 constraints/req -> 0.63, 1.0 -> 0.86, 2.0 -> 0.98
        density = total_constraints / total_reqs
        constraint_density = 1.0 - math.exp(-2.0 * density)

        # Negative Space Coverage (NSC)
        # = presence of explicit boundaries (saturating)
        # Steepness k=5.0: 0.1 negatives/req -> 0.39, 0.2 -> 0.63, 0.5 -> 0.92
        negative_ratio = total_negative / total_reqs
        negative_space_coverage = 1.0 - math.exp(-5.0 * negative_ratio)

...
def migrate(palace_path: str, dry_run: bool = False, confirm: bool = False):
    ...
    try:
        col = ChromaBackend().get_collection(palace_path, "castle_drawers")
        count = col.count()
    ...
    temp_palace = tempfile.mkdtemp(prefix="castle_migrate_")
    print(f"  Creating fresh palace in {temp_palace}...")
    fresh_backend = ChromaBackend()
    col = fresh_backend.get_or_create_collection(temp_palace, "castle_drawers")

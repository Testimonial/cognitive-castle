...
Reads castle.yaml from the project directory to know the wing + rooms.
...
def load_config(project_dir: str) -> dict:
    """Load castle.yaml from project directory (falls back to mempal.yaml)."""
    import yaml

    resolved_project_dir = Path(project_dir).expanduser().resolve()
    config_path = resolved_project_dir / "castle.yaml"
...
_ENTITY_REGISTRY_PATH = os.path.join(os.path.expanduser("~"), ".castle", "known_entities.json")
...
def _refresh_known_entities_cache() -> None:
    """Reload ``~/.castle/known_entities.json`` into the module cache if

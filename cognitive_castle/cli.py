...
def _run_pass_zero(project_dir, palace_dir, llm_provider) -> dict:
    ...
    result to ``<palace>/.castle/origin.json``.
    ...
    # gemma3:e4b) can return a wrong likely_ai_dialogue/confidence call
    ...
    origin_path = Path(palace_dir).expanduser() / ".castle" / "origin.json"
    try:
...
def cmd_init(args):
    ...
    # flag and always used ~/.castle. Mirror the env-var pattern used by
    ...
    if not getattr(args, "no_llm", False):
        ...
        provider_model = getattr(args, "llm_model", "gemma3:e4b") or "gemma3:e4b"
        try:
    ...
    # <palace>/.castle/origin.json and supplies corpus context to the
    ...
    if total > 0:
        ...
        if confirmed["people"] or confirmed["projects"] or confirmed.get("topics"):
            ...
            # ``castle.yaml`` so the miner's tunnel lookup hits the
    ...
    _ensure_castle_files_gitignored(args.dir)
...
def cmd_compress(args):
    ...
    if not args.dry_run:
        try:
            ...
            print(
                f"  Stored {len(compressed_entries)} compressed drawers in 'castle_closets' collection."
            )
        except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--palace",
        default=None,
        help="Where the palace lives (default: from ~/.castle/config.json or ~/.castle/palace)",
    )
    ...
    p_init.add_argument(
        ...
        help=(
            ...
            "(CASTLE_ENTITY_LANGUAGES env var or config.json), or 'en'. "
    ...
    p_init.add_argument(
        "--llm-model",
        default="gemma3:e4b",
        help="Model name for the chosen provider (default: gemma3:e4b for Ollama).",
    )
    p_init.add_argument(
        "--llm-endpoint",
        default=None,

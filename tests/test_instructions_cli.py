"""Tests for cognitive_castle.instructions_cli — instruction text output."""
...
def test_run_instructions_missing_md_file(capsys, tmp_path):
    ...
    with patch("cognitive_castle.instructions_cli.INSTRUCTIONS_DIR", tmp_path):
        with patch("cognitive_castle.instructions_cli.AVAILABLE", ["fakecmd"]):
            with pytest.raises(SystemExit) as exc_info:

...
accident. The fix adds `_ensure_castle_files_gitignored()` which appends
the two filenames to `.gitignore` when `<dir>` is a git repository.
"""

from pathlib import Path

from cognitive_castle.cli import _ensure_castle_files_gitignored
...
def test_no_op_when_not_a_git_repo(tmp_path):
    assert _ensure_castle_files_gitignored(tmp_path) is False
    assert not (tmp_path / ".gitignore").exists()


def test_creates_gitignore_with_both_entries(tmp_path):
    _git_init(tmp_path)
    assert _ensure_castle_files_gitignored(tmp_path) is True
    contents = (tmp_path / ".gitignore").read_text()
    assert "castle.yaml" in contents
...
def test_appends_only_missing_entries(tmp_path):
    ...
    assert contents.count("castle.yaml") == 1

...
def test_llm_model_default_matches_cmd_init_default():
    """Default tracks cmd_init's hardcoded `--llm-model` default.

    This test verifies the config-driven and CLI-driven paths stay in
    sync; if a future change drifts them apart, users would get
    different LLM defaults via `castle init` vs in-process config
    loading. Both must match.
    """
    cfg = _make_config_with_file_config({})
...
# ── KG enrichment config (Task 1: kg-enrichment-during-ingest) ──────────────

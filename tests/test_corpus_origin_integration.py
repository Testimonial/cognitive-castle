...
def _stub_cfg(palace_dir: Path):
    """Build a CognitiveCastleConfig stub whose palace_path points at tmp space.
...
def test_init_pass_zero_writes_origin_json_to_palace(ai_dialogue_corpus: Path, tmp_path: Path):
    ...
    with (
        patch("cognitive_castle.cli.CognitiveCastleConfig", return_value=_stub_cfg(palace)),
...
def test_integration_cmd_init_runs_pass_zero_to_pass_four_in_order(
    ...
    real_run_pass_zero = __import__(
        "cognitive_castle.cli", fromlist=["_run_pass_zero"]
    )._run_pass_zero

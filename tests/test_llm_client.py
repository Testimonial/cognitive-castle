...
def test_http_post_json_success():
    ...
    with patch("cognitive_castle.llm_client.urlopen", return_value=mock_resp):
...
def test_http_post_json_http_error_wraps_as_llm_error():
    ...
    with patch("cognitive_castle.llm_client.urlopen", side_effect=err):
...
def test_http_post_json_url_error_wraps_as_llm_error():
    ...
    with patch("cognitive_castle.llm_client.urlopen", side_effect=URLError("conn refused")):
...
def test_ollama_classify_empty_content_raises():
    with patch("cognitive_castle.llm_client.urlopen", return_value=_mock_ollama_chat_response("")):

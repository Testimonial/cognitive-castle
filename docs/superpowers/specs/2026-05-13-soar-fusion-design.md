...
**Date:** 2026-05-13
**Status:** Revised 4× after review (2026-05-13) — ready for implementation plan
...
- **2026-05-13 (post-review #4):** CLI flow ambiguity + MCP comment polish:
  20. **CLI flow ambiguity resolved.** The cli.py Components row said "Pass through to `search()` (which then propagates to the search dict)" — but `search()` is print-only and doesn't return a dict. Plan-writer would have hit the architectural question on day one. Spec now states: when `--soar-boost` is on, `cmd_search` calls `search_memories()` directly (the dict-returning function), applies boosts to `result["results"]`, re-sorts, then formats output the same way `search()` would. Default path (no flag) is unchanged.
  21. **MCP comment clarified.** Old wording said LLM-rerank was "applied earlier in handler from PR #3" but LLM-rerank actually lives INSIDE `searcher.py:_new_pipeline_search` — not in the handler. The handler just passes a flag. Wording corrected to "search_memories returns LLM-reranked hits directly; SOAR post-processes in the handler."
  22. **cli.py LOC bumped** 25 → 30 to absorb the search_memories-direct-call branch. Total ~680 → ~685.
...
**Total:** ~685 LOC across 2 new modules + 1 new rule file + 1 new test file + 4 existing files. (Components row sums: 260+35+30+30+30+200+30+20+45+5 = 685. cli.py grew 25 → 30 in review-4 to absorb the search_memories-direct-call branch.)
...
## Spec self-review (post-revision #4, 2026-05-13)
...
11. **Post-review #4 findings addressed:**
    - ✅ **CLI flow ambiguity resolved:** `cmd_search` calls `search_memories()` directly (not `search()`) when `--soar-boost` is on, applies boosts to `result["results"]`, re-sorts, formats output identical to `search()`. Default path unchanged.
    - ✅ MCP handler comment clarified: LLM-rerank lives INSIDE `searcher.py:_new_pipeline_search`, not in the handler. The handler sees LLM-reranked hits directly and post-processes with SOAR.
    - ✅ cli.py LOC bumped 25 → 30 to absorb the search_memories-direct-call branch. Total `~680 → ~685` with new per-row sum.
    - ⊘ Skipped (plan-writer can resolve): Soar production syntax example (derivable from prior bridge), `@pytest.mark.soar` marker registration in pyproject.toml (would cause warnings only), `recently-accessed` string-vs-symbol convention (plan-writer discovers at impl time).

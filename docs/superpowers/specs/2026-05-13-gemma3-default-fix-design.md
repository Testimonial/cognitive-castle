...
Castle's documented default LLM model is `gemma3:e4b` — a tag that **does not exist in Ollama's registry**. Verified live during PR #3 review (2026-05-13): `ollama show gemma3:e4b` returns "model not found." The real tags published at `ollama.com/library/gemma3` are: `1b`, `4b`, `12b`, `27b`, `270m`, `latest` (plus `-it-qat` and `-cloud` variants).
...
**Total: 9 substitutions + 1 paragraph removal.** No LOC delta in production code.
...
2. **Internal consistency:** Architecture, Data Flow, Testing, and Acceptance all reference:
   ...
   - 9 substitutions across 4 files (config.py × 2, cli.py × 4, test_config.py × 2, README.md × 1 paragraph removal)
   - Cosmetic test strings (`tests/test_llm_client.py` + `tests/test_corpus_origin_integration.py`) explicitly out of scope
3. **Scope:** Single-purpose string-replacement bug fix. ~9 sites in 4 files. Not decomposable into sub-projects.
4. **Ambiguity:** "Cosmetic test strings out of scope" stated explicitly in 3 places (Non-goals, Architecture footer, Acceptance #1). Variant choice (`4b` over `1b`/`latest`/`12b`/`27b`) stated explicitly in a comparison table with reasoning.
5. **Empirical grounding:**
   - Real Ollama gemma3 tags verified at `ollama.com/library/gemma3` (1b, 4b, 12b, 27b, 270m, latest)
   - `gemma3:4b` confirmed real (subset of the verified tag list)
   - `gemma3:e4b` confirmed broken via `ollama show gemma3:e4b` → "model not found"
   - 9 production sites located via `grep -rn "gemma3:e4b"` across the repo

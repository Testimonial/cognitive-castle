...
21. Production string-literal references to `"cognitive-castle"` (the PyPI distribution / brand name) at `cli.py:993` (argparse default), `miner.py:989`, `miner.py:1044`, `convo_miner.py:383`, `mcp_server.py:1643`, `entity_registry.py:34`, `project_scanner.py:572`, `project_scanner.py:642` are UNCHANGED. These are production brand-name strings, not in PR #D's scope. The spec's choice to use `cognitive-castle/` in doc prose paths is consistent with this existing production brand usage — but the production strings themselves stay untouched.
